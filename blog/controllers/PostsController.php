...
class PostsController extends BaseController
{
    ...
    public function create()
    {
        if($this->isPost) {
            ...
            if(strlen($title) < 1) {
                $this->setValidationError("post_title", "Title is empty or is too short.");
            }
            $content = $_POST['post_content'];
            if(strlen($content) < 1) {
                $this->setValidationError("post_content", "Content is empty or is too short.");
            }
            if($this->formValid()) {
    ...
    public function edit(int $id)
    {
        if($this->isPost) {
            $title = $_POST['post_title'];
            if(strlen($title) < 1) {
                $this->setValidationError("post_title", "Title is empty or too short.");
            }
            $content = $_POST['post_content'];
            if(strlen($content) < 1) {
                $this->setValidationError("post_content", "Title is empty or too short.");
            }
